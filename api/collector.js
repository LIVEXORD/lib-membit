...
async function releaseLock() {
  try { await redis.del(LOCK_FLUSH); } catch (e) { console.warn("releaseLock", e?.message ?? e); }
}
...
async function recoverProcessingList() {
  ...
  console.warn(`Recovered ${moved} items from processing list back to input list`);
  return moved;
}
...
async function ensureReadStructures() {
  ...
  if (totalRead > 0) {
    ...
    if (!valid) {
      console.warn("READ_LIST contains non-json placeholder — clearing read:list & read:seen");
  ...
  if (totalAfter === 0) {
    ...
    if (moved > 0) {
      ...
      console.warn("ensureReadStructures: found input list but moved 0 items — leaving INPUT_LIST intact");
    }
  ...
  while (true) {
    ...
    if (!id) {
      try {
        ...
        console.error("Failed pushing noid parsed item to READ_LIST", e?.message ?? e);
        await redis.lpush(INPUT_LIST, s);
      }
      await redis.lrem(PROCESSING_LIST, 1, s);
      continue;
...
app.get("/collector", async (req, res) => {
  ...
  try {
    ...
    const batchSize = Math.min(
      Math.max(parseInt(q.batch_size || String(READ_BATCH_SIZE), 10) || READ_BATCH_SIZE, 1),
      500
    );
