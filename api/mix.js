...
async function loadData() {
  const now = Date.now();
  if (now - globalThis._cacheData.lastUpdate < CACHE_DURATION && globalThis._cacheData.data.length > 0) {
    return {
  ...
  if (!gistRes.ok) throw new Error(`Failed to fetch gist metadata: ${gistRes.status}`);
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (Date.now() - globalThis._cacheData.lastUpdate > CACHE_DURATION) {
        ...
        return res.status(201).json({ message: skipped.length ? 'Some skipped (dupes)' : 'All added', added, skipped });
      } else {
        // Store in memory cache until next push
        globalThis._cacheData.data = newData;
        return res.status(202).json({ message: 'Cached temporarily. Will push soon.', added, skipped });
      }
    }
